...
class Distance(CollectiveVariable):
    ...
    def __init__(self, atoms: list[int], name: str | None = None) -> None:
        ...
        if len(atoms) != 2:
            raise ValueError(
                f"Distance requires exactly 2 atoms, got {len(atoms)}."
            )
        self._atoms = atoms
        super().__init__(name)
...
class Angle(CollectiveVariable):
    ...
    def __init__(self, atoms: list[int], name: str | None = None) -> None:
        ...
        if len(atoms) != 3:
            raise ValueError(
                f"Angle requires exactly 3 atoms, got {len(atoms)}."
            )
        self._atoms = atoms
        super().__init__(name)
...
class Dihedral(CollectiveVariable):
    ...
    def __init__(self, atoms: list[int], name: str | None = None) -> None:
        ...
        if len(atoms) != 4:
            raise ValueError(
                f"Dihedral requires exactly 4 atoms, got {len(atoms)}."
            )
        self._atoms = atoms
        super().__init__(name)
...
class Restraint(BiasAction):
    ...
    def __init__(
        self, cv: CollectiveVariable, center: float, kappa: float
    ) -> None:
...
class Metadynamics(BiasAction):
    ...
    Example::
        ...
        d = Distance([0, 1])
        metad = Metadynamics(d, sigma=0.5, height=0.005, pace=100,
                             biasfactor=10.0)
    """
...
class Print(BiasAction):
    ...
    def to_plumed_lines(self) -> list[str]:
        ...
        labels = [
            cv.name if isinstance(cv, CollectiveVariable) else cv
            for cv in self._cvs
        ]
        arg = ",".join(labels)
