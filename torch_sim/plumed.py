...
class PlumedModel(ModelInterface):
    ...
    def forward(
        self, state: SimState | StateDict, **kwargs: Any
        ...
        Args:
            state: Simulation state or state dictionary.
        ...
        for i in range(n_systems):
            ...
            pos_i = (
                state.positions[mask].detach().cpu().numpy().astype(np.float64)
            )
            masses_i = (
                state.masses[mask].detach().cpu().numpy().astype(np.float64)
            )
