"""AuPt binary alloy multi-cell MC at 1000 K.
...
phase at 1000 K, consistent with the known Au-Pt miscibility gap.
...
orb_ff = pretrained.orb_v3_conservative_inf_omat(device=device, precision="float32-highest")
...
trajs = [
    Trajectory(str(RESULTS_DIR / "aupt_1000k_cell1.traj"), "w"),
    Trajectory(str(RESULTS_DIR / "aupt_1000k_cell2.traj"), "w"),
]
...
print(f"\nData saved to {RESULTS_DIR / 'aupt_1000k.npz'}")
print(f"Trajectories saved to {RESULTS_DIR}/aupt_1000k_cell1.traj and aupt_1000k_cell2.traj")
...
axes[0].set_title("AuPt at 1000 K  —  Multi-cell MC (ORB v3)")
axes[0].legend()
axes[0].set_ylim(0, 1)
...
fig.tight_layout()
fig.savefig(RESULTS_DIR / "aupt_1000k.png", dpi=150)
print(f"Plot saved to {RESULTS_DIR / 'aupt_1000k.png'}")
plt.close(fig)
