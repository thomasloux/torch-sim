"""aupt_half binary alloy multi-cell MC at 600 K.

Reproduces the aupt_half simulation from:
...
def make_fcc_alloy(a: float, n_minor: int, Z_major: int, Z_minor: int,
                   repeat: tuple[int, int, int] = (3, 3, 3)) -> "ase.Atoms":  # noqa: F821
    """FCC supercell with n_minor atoms of Z_minor and the rest Z_major."""
    atoms = bulk(
        {Z_AU: "Au", Z_PT: "Pt"}[Z_major],
        "fcc", a=a, cubic=True
    ).repeat(repeat)   # 4 * 8 = 32 atoms
    idx = rng_np.choice(len(atoms), size=n_minor, replace=False)
    atoms.numbers[idx] = Z_minor
    return atoms

# Cell 1 — Au-rich: 24 Au + 8 Pt
a1     = 0.5 * a_au + 0.5 * a_pt   # ≈ 4.039 Å
cell1_ase = make_fcc_alloy(a1, n_minor=16,  Z_major=Z_AU, Z_minor=Z_PT)

# Cell 2 — Pt-rich: 8 Au + 24 Pt
a2     = 0.5 * a_au + 0.5 * a_pt   # ≈ 3.961 Å
cell2_ase = make_fcc_alloy(a2, n_minor=16,  Z_major=Z_PT, Z_minor=Z_AU)
...
trajs = [
    Trajectory(str(RESULTS_DIR / "aupt_600k_half_cell1.traj"), "w"),
    Trajectory(str(RESULTS_DIR / "aupt_600k_half_cell2.traj"), "w"),
]
...
print("\n" + "=" * 60)
print("RESULTS: aupt_half at 600 K")
print("=" * 60)
...
# ── save data ─────────────────────────────────────────────────────────────────
np.savez(
    RESULTS_DIR / "aupt_half_600k.npz",
...
print(f"\nData saved to {RESULTS_DIR / 'aupt_half_600k.npz'}")
print(f"Trajectories saved to {RESULTS_DIR}/aupt_600k_half_cell1.traj and aupt_600k_half_cell2.traj")
...
axes[0].set_title("aupt_half at 600 K  —  Multi-cell MC (ORB v3)")
axes[0].legend()
axes[0].set_ylim(0, 1)
...
fig.tight_layout()
fig.savefig(RESULTS_DIR / "aupt_half_600k.png", dpi=150)
print(f"Plot saved to {RESULTS_DIR / 'aupt_half_600k.png'}")
plt.close(fig)
